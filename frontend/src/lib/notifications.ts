...
export async function sendNativeNotification(notification: AppNotification) {
  ...
  await LocalNotifications.schedule({ notifications: [{ id: Date.now() % 2147483647, title: notification.title, body: notification.body, smallIcon: 'ic_stat_mathan', extra: notification.url ? { url: notification.url } : undefined, schedule: { at: new Date(Date.now() + 250) } }] });
}
