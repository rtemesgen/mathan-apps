            ...
            <div className="space-y-3">
              ...
              <div className="border border-slate-200 rounded-xl overflow-hidden divide-y divide-slate-100 bg-white">
                ...
                {[...employee.salaryHistory]
                  ...
                  {confirmDeleteRaise === change.id && <div className="border-t border-red-100 bg-red-50 px-3.5 py-2.5 flex items-center justify-between gap-2"><span className="text-xs font-semibold text-red-800">Delete this salary raise?</span><span className="flex gap-1.5"><button type="button" onClick={() => setConfirmDeleteRaise(null)} className="rounded-md border border-red-200 bg-white px-2.5 py-1 text-[11px] font-bold text-zinc-700">Cancel</button><button type="button" onClick={() => { onDeleteRaise?.(employee.id, change.id); setConfirmDeleteRaise(null); }} className="rounded-md bg-red-700 px-2.5 py-1 text-[11px] font-bold text-white">Delete</button></span></div>}
                  </React.Fragment>
                ))}
              </div>
            </div>
          ...
          {activeTab === 'withdrawals' && (
            <div className="space-y-3">
              {empTransactions.length === 0 ? (
                ...
                <>
                {confirmDeleteTransaction && <div className="rounded-xl border border-red-200 bg-red-50 px-3 py-2.5 flex items-center justify-between gap-2"><span className="text-xs font-semibold text-red-800">Delete this payment record?</span><span className="flex gap-1.5"><button type="button" onClick={() => setConfirmDeleteTransaction(null)} className="rounded-md border border-red-200 bg-white px-2.5 py-1 text-[11px] font-bold text-zinc-700">Cancel</button><button type="button" onClick={() => { (onRemoveTransaction ?? onDeleteTransaction)?.(confirmDeleteTransaction); setConfirmDeleteTransaction(null); }} className="rounded-md bg-red-700 px-2.5 py-1 text-[11px] font-bold text-white">Delete</button></span></div>}
