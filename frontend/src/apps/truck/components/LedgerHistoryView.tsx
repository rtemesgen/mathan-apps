          ...
          <table className="w-full text-left text-xs">
            <thead className="bg-[#f8f6f0] border-b border-[#e5dfd2] text-[#787672] uppercase text-[9px] font-bold tracking-wider">
              <tr>
                ...
                <th className="py-2.5 px-3">Partner / Truck</th>
            ...
            <tbody className="divide-y divide-[#f0ebd9] font-medium text-[#1c1d1f]">
              {filteredTx.length === 0 ? (
                <tr>
                  <td colSpan={6} className="py-8 text-center text-[#8c8880] italic">
                ...
                filteredTx.map((tx) => {
                  const isPositive =
                    tx.type === 'INCOME' || tx.type === 'CAPITAL_INJECTION';
                  return (
                    <tr key={tx.id} className="hover:bg-[#faf8f5]">
                      <td className="py-2 px-3 whitespace-nowrap text-[#787672]">
                        {formatDate(tx.date)}
                      </td>
                      ...
                      <td className="py-2 px-3">
                        <div className="font-bold text-xs">{tx.category}</div>
                        <div className="text-[10px] text-[#787672] truncate max-w-xs">
                          {tx.description}
                        </div>
                      </td>
                      <td className="py-2 px-3 whitespace-nowrap text-[#4a4843]">
