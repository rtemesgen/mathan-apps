...
export function RenameBookModal({ book, onClose, onSave }: { book: Book | null; onClose: () => void; onSave: (bookId: string, name: string) => void }) {
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-3" onClick={onClose}>
    <form className="w-full max-w-sm rounded-xl border border-[#E6E2D6] bg-white p-4 shadow-xl" onClick={(event) => event.stopPropagation()} onSubmit={(event) => { event.preventDefault(); if (name.trim()) onSave(book.id, name.trim()); }}>
      ...
      <div className="mt-4 flex justify-end gap-2"><button type="button" onClick={onClose} className="rounded-lg px-3 py-2 text-xs font-bold text-[#6B7280] hover:bg-[#F7F5EE]">Cancel</button><button className="rounded-lg bg-[#121212] px-3 py-2 text-xs font-bold text-white">Save</button></div>
    </form>
  </div>;
}
