...
export function RenameBookModal({ book, onClose, onSave }: { book: Book | null; onClose: () => void; onSave: (bookId: string, name: string) => void }) {
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-3" onClick={onClose}>
    <form className="w-full max-w-sm rounded-xl border border-[#E6E2D6] bg-white p-4 shadow-xl" onClick={(event) => event.stopPropagation()} onSubmit={(event) => { event.preventDefault(); if (name.trim() && !submitting) void runAction({ operation: () => onSave(book.id, name.trim()), errorMessage: 'Could not rename the Cash Book. Your entry was kept.' }).then(onClose).catch(() => undefined); }}>
