...
export function ImportBookModal({ isOpen, onClose, onImport }: { isOpen: boolean; onClose: () => void; onImport: (payload: ImportPayload) => void }) {
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-3" onClick={onClose}>
    <div className="w-full max-w-md rounded-xl border border-[#E6E2D6] bg-white p-4 shadow-xl" onClick={(event) => event.stopPropagation()}>
      ...
      <div className="mt-4 flex justify-end gap-2"><button type="button" onClick={onClose} className="rounded-lg px-3 py-2 text-xs font-bold text-[#6B7280] hover:bg-[#F7F5EE]">Cancel</button><button type="button" disabled={!parsed.length || busy} onClick={() => onImport(parsed)} className="rounded-lg bg-[#121212] px-3 py-2 text-xs font-bold text-white disabled:cursor-not-allowed disabled:opacity-40">Import</button></div>
    </div>
  </div>;
}
