...
const template = 'Book Name,Description,Category,Currency,Opening Balance,Date,Type,Amount,Remark,Payment Mode\nRetail Shop,Daily cash book,Business,$,1000,2026-08-11 09:00,Cash In,1500,Sale,Cash\nRetail Shop,Daily cash book,Business,$,1000,2026-08-11 17:00,Cash Out,200,Transport,Cash\n';
...
export function ImportBookModal({ isOpen, onClose, onImport }: { isOpen: boolean; onClose: () => void; onImport: (payload: ImportPayload) => void | Promise<void> }) {
  ...
  const resetImportState = () => {
    setFileName(''); setParsed([]); setBusy(false); setError('');
    if (inputRef.current) inputRef.current.value = '';
  };
  const closeModal = () => { if (submitting) return; resetImportState(); onClose(); };
  useEffect(() => { if (!isOpen) resetImportState(); }, [isOpen]);
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-3" onClick={closeModal}>
    <div className="w-full max-w-md rounded-xl border border-[#E6E2D6] bg-white p-4 shadow-xl" onClick={(event) => event.stopPropagation()}>
      <div className="flex items-center justify-between"><div><h2 className="flex items-center gap-2 text-sm font-bold"><FileUp className="h-4 w-4" /> Import books</h2><p className="mt-1 text-[11px] text-[#6B7280]">Choose a PDF, Excel, or CSV file.</p></div><button type="button" onClick={closeModal}><X className="h-4 w-4 text-[#6B7280]" /></button></div>
      ...
      <p className="mt-2 text-[10px] leading-4 text-[#6B7280]">Supports Cash Book, Payroll payment, and Truck transaction exports in CSV or Excel, plus the template. Cash Book statements preserve opening balance; summary-only reports cannot create entries.</p>
      ...
      <div className="mt-4 flex justify-end gap-2"><button type="button" onClick={closeModal} disabled={submitting} className="rounded-lg px-3 py-2 text-xs font-bold text-[#6B7280] hover:bg-[#F7F5EE] disabled:opacity-50">Cancel</button><button type="button" disabled={!parsed.length || busy || submitting} onClick={() => void runAction({ operation: async () => { await onImport(parsed); resetImportState(); onClose(); }, successMessage: 'Cash Book imported successfully.', errorMessage: 'Could not import the Cash Book. Your file was kept.' })} className="rounded-lg bg-[#121212] px-3 py-2 text-xs font-bold text-white disabled:cursor-not-allowed disabled:opacity-40">{submitting ? 'Importing…' : 'Import'}</button></div>
    </div>
  </div>;
}
