...
export function AddMembersModal({ book, onClose }: { book: Book | null; onClose: () => void }) {
  ...
  const addMember = async (member: Member) => {
    if (!workspace || member.book_permission !== 'none') return;
    ...
    if (addError) setError(addError.message); else { setMembers((current) => current.map((item) => item.user_id === member.user_id ? { ...item, book_permission: 'edit' } : item)); setNotice(`${member.display_name || member.email} can now use Cash Book.`); }
    setBusy(false);
  };
