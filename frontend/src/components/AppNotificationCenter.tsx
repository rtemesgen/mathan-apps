...
export function AppNotificationCenter() {
  ...
  const loadDurable = async () => { if (!user) { setDurable([]); return; } const { data } = await supabase.rpc('list_my_notifications', { target_limit: 50 }); setDurable((data as DurableNotification[] | null) ?? []); };
  useEffect(() => {
    ...
    return () => { window.removeEventListener('mathan:notification', handle); window.removeEventListener('mathan:sync-conflict', conflictHandle); window.removeEventListener('mathan:sync-status', syncHandle); if (timer) window.clearTimeout(timer); };
  }, []);
  ...
  const markRead = async (item: DurableNotification) => { await supabase.rpc('mark_notification_read', { target_id: item.id }); setDurable((current) => current.map((entry) => entry.id === item.id ? { ...entry, read_at: new Date().toISOString() } : entry)); };
  const markAllRead = async () => { await supabase.rpc('mark_all_notifications_read'); setDurable((current) => current.map((item) => ({ ...item, read_at: item.read_at ?? new Date().toISOString() }))); };
  if (!notification && !permissionMessage && !conflict && !user) return null;
  return <>
    {user && <div className="fixed right-4 top-3 z-[210]"><button type="button" aria-label="Notifications" onClick={() => setInboxOpen((current) => !current)} className="relative rounded-xl border border-[#e6e2d6] bg-white p-2.5 text-zinc-700 shadow-sm"><Bell className="h-4 w-4" />{unread > 0 && <span className="absolute -right-1 -top-1 flex h-4 min-w-4 items-center justify-center rounded-full bg-red-600 px-1 text-[9px] font-bold text-white">{unread > 9 ? '9+' : unread}</span>}</button>{inboxOpen && <section className="absolute right-0 mt-2 w-[min(22rem,calc(100vw-2rem))] overflow-hidden rounded-2xl border border-[#e6e2d6] bg-white shadow-2xl"><div className="flex items-center justify-between border-b border-[#eeeae0] p-3"><p className="text-sm font-extrabold">Notifications</p><button type="button" onClick={() => void markAllRead()} className="text-[10px] font-bold text-emerald-800">Mark all read</button></div><div className="max-h-80 overflow-y-auto">{durable.length ? durable.map((item) => <button type="button" key={item.id} onClick={() => { void markRead(item); if (item.route) window.location.assign(item.route); }} className={`block w-full border-b border-[#f0eee7] p-3 text-left hover:bg-[#faf9f5] ${item.read_at ? '' : 'bg-emerald-50/50'}`}><p className="text-xs font-bold">{item.title}</p><p className="mt-1 text-[11px] leading-4 text-zinc-600">{item.body}</p><p className="mt-1 text-[10px] text-zinc-400">{new Date(item.created_at).toLocaleString()}</p></button>) : <p className="p-4 text-xs text-zinc-500">No notifications yet.</p>}</div></section>}</div>}
    {user && <div className={`fixed bottom-3 left-3 z-[205] rounded-full border px-3 py-1.5 text-[10px] font-bold shadow-sm ${syncStatus === 'synced' ? 'border-emerald-200 bg-emerald-50 text-emerald-800' : syncStatus === 'conflict' ? 'border-red-200 bg-red-50 text-red-800' : 'border-amber-200 bg-amber-50 text-amber-900'}`}>{syncStatus === 'synced' ? 'Synced' : syncStatus === 'syncing' ? 'Saving…' : syncStatus === 'offline' ? 'Offline · saved locally' : syncStatus === 'retry' ? 'Retry pending' : 'Conflict needs review'}</div>}
