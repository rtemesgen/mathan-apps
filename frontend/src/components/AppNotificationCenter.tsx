...
export function AppNotificationCenter() {
  ...
  const [syncStatus, setSyncStatus] = useState<'synced' | 'syncing' | 'offline' | 'retry' | 'conflicted' | 'error'>('synced');
  ...
  useEffect(() => {
    ...
    const syncHandle = (event: Event) => setSyncStatus((event as CustomEvent<{ status: 'synced' | 'syncing' | 'offline' | 'retry' | 'conflicted' | 'error' }>).detail.status);
  ...
  return <>
    ...
    {user && location.pathname === '/' && <div className={`fixed bottom-3 left-3 z-[205] rounded-full border px-3 py-1.5 text-[10px] font-bold shadow-sm ${syncStatus === 'synced' ? 'border-emerald-200 bg-emerald-50 text-emerald-800' : syncStatus === 'conflicted' || syncStatus === 'error' ? 'border-red-200 bg-red-50 text-red-800' : 'border-amber-200 bg-amber-50 text-amber-900'}`}>{syncStatus === 'synced' ? 'Synced' : syncStatus === 'syncing' ? 'Saving…' : syncStatus === 'offline' ? 'Offline · saved locally' : syncStatus === 'retry' ? 'Retry pending' : syncStatus === 'error' ? 'Sync error' : 'Conflict needs review'}</div>}
