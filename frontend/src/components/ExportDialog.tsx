...
export function ExportDialog({ open, onClose, context }: { open: boolean; onClose: () => void; context: ExportContext | null }) {
  ...
  const run = async () => {
    ...
    setBusy(true);
    try {
      const options: ExportBuildOptions = { ...context.activeFilters, detail, entityId: entityId || undefined, ownerId: ownerId || undefined, transactionType: transactionType || undefined, startDate: startDate || undefined, endDate: endDate || undefined };
  ...
  const showFilters = entities.length > 0 || owners.length > 0 || context.activeFilters?.startDate !== undefined || context.activeFilters?.endDate !== undefined || !!context.availableTransactionTypes?.length;
