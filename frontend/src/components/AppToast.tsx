...
import { CheckCircle2, CircleAlert, CloudOff, LoaderCircle, X } from 'lucide-react';
...
export function AppToast() {
  ...
  return <div role={critical ? 'alert' : 'status'} className={`pointer-events-auto fixed left-3 right-3 top-16 z-[220] max-h-32 overflow-y-auto rounded-xl border px-4 py-3 text-xs font-semibold shadow-lg animate-in fade-in slide-in-from-top-2 sm:left-auto sm:right-4 sm:top-4 sm:max-w-[24rem] ${critical ? 'border-red-200 bg-red-50 text-red-800' : working ? 'border-amber-200 bg-amber-50 text-amber-900' : info ? 'border-sky-200 bg-sky-50 text-sky-800' : 'border-emerald-200 bg-emerald-50 text-emerald-800'}`}><div className="flex items-center gap-2"><Icon className={`h-4 w-4 shrink-0 ${toast.state === 'saving' ? 'animate-spin' : ''}`} /><span className="min-w-0 flex-1">{toast.message}</span><button type="button" onClick={() => setToast(null)} aria-label="Dismiss notification" className="rounded p-0.5"><X className="h-4 w-4" /></button></div>{toast.conflict && <div className="mt-3 flex gap-2"><button type="button" onClick={() => window.location.reload()} className="rounded-lg bg-red-900 px-2.5 py-1.5 text-[11px] font-bold text-white">Reload remote</button><button type="button" onClick={() => setToast(null)} className="rounded-lg border border-red-300 px-2.5 py-1.5 text-[11px] font-bold">Keep local</button></div>}</div>;
}
