  ...
  return <main className="erp-app mx-auto max-w-6xl px-4 py-5 sm:px-6 sm:py-7">
    ...
    <div className="flex flex-col gap-5 lg:flex-row">
      ...
      <div className="min-w-0 flex flex-1 flex-col">
        ...
        {activeSection === 'profile' && <div className="order-1 space-y-4"><WorkspaceInvitations /><section className="rounded-2xl border border-emerald-200 bg-emerald-50 p-4"><h2 className="font-serif text-lg font-bold text-emerald-950">Guest data</h2><p className="mt-1 text-xs leading-5 text-emerald-800">If this device has local guest companies, you can merge them into an eligible cloud company without replacing its existing records.</p><button type="button" onClick={() => { sessionStorage.removeItem('mathan_guest_import_deferred'); window.location.href = '/'; }} className="mt-3 rounded-xl bg-emerald-800 px-4 py-2.5 text-xs font-bold text-white">Check for guest data</button></section><SecuritySettingsCard password={password} editing={editingSecurity} showPassword={showPassword} busy={busy} onEdit={() => setEditingSecurity(true)} onCancel={() => { setPassword(''); setShowPassword(false); setEditingSecurity(false); }} onSubmit={updatePassword} onTogglePassword={() => setShowPassword((current) => !current)} setPassword={setPassword} /><SessionSecurityCard onNotice={setNotice} onError={setError} /><BackupRecoveryCard /><PersonalDataCard userId={user?.id} isOwner={isOwner} onTransferCompany={() => setActiveSection('company')} onNotice={setNotice} onError={setError} /><MyCompanyMemberships /></div>}
