...
export function SettingsPage() {
  ...
  const [activeSection, setActiveSection] = useState<'company' | 'apps' | 'invites' | 'people' | 'profile' | 'security' | 'approvals' | 'trash'>(requestedSection === 'invites' ? requestedSection : 'profile');
  ...
  return <main className="erp-app mx-auto max-w-6xl px-4 py-5 sm:px-6 sm:py-7">
    ...
    <div className="flex flex-col gap-5 lg:flex-row">
      ...
      <div className="min-w-0 flex flex-1 flex-col">
        ...
        {activeSection === 'company' && isOwner && <div className="space-y-4"><WorkspaceInvitations /><CompanySettingsCard companyName={companyName} accentColor={accentColor} editing={editingCompany} busy={busy} onEdit={() => setEditingCompany(true)} onCancel={() => { setCompanyName(workspace?.name ?? ''); setAccentColor(workspace?.accent_color ?? '#54623E'); setEditingCompany(false); }} onSubmit={saveCompany} setCompanyName={setCompanyName} setAccentColor={setAccentColor} /></div>}
        {activeSection === 'apps' && isOwner && <section className="space-y-3">{(['book', 'payroll'] as AppId[]).map((app) => <div key={app} className="flex items-center justify-between rounded-2xl border border-[#e6e2d6] bg-white p-4 shadow-sm"><div className="flex items-center gap-3"><span className="flex h-9 w-9 items-center justify-center rounded-xl bg-emerald-50 text-emerald-700"><Power className="h-4 w-4" /></span><div><p className="text-sm font-bold">{appNames[app]}</p><p className="text-xs text-zinc-500">{appAccess[app].enabled ? 'Available to permitted members' : 'Disabled and hidden'}</p></div></div><div className="flex items-center gap-3"><span className="text-[10px] font-extrabold uppercase tracking-wider text-zinc-500">{appAccess[app].enabled ? 'On' : 'Off'}</span><AppToggle enabled={appAccess[app].enabled} onChange={() => { if (appAccess[app].enabled) setAppToDisable(app); else void toggleApp(app); }} /></div></div>)}</section>}
        ...
        {activeSection === 'apps' && isOwner && <section className="rounded-2xl border border-amber-200 bg-amber-50/60 p-4 shadow-sm"><div className="flex items-center justify-between gap-3"><div><p className="text-sm font-bold">{appNames.truck}</p><p className="text-xs text-zinc-600">{appAccess.truck.enabled ? 'Available to permitted members' : 'Disabled and hidden'}</p></div><div className="flex items-center gap-3"><span className="text-[10px] font-extrabold uppercase tracking-wider text-zinc-600">{appAccess.truck.enabled ? 'On' : 'Off'}</span><AppToggle enabled={appAccess.truck.enabled} onChange={() => { if (appAccess.truck.enabled) setAppToDisable('truck'); else void toggleApp('truck'); }} /></div></div></section>}
