...
export function SettingsPage() {
  ...
  type SettingsSection = 'company' | 'apps' | 'invites' | 'appdata' | 'people' | 'profile' | 'security' | 'approvals' | 'trash';
  ...
  const navItems = [
    ...
    ...(isOwner ? [{ id: 'company' as const, label: 'Company' }, { id: 'apps' as const, label: 'Apps' }, { id: 'invites' as const, label: 'Invitations' }, { id: 'appdata' as const, label: 'App data' }, { id: 'approvals' as const, label: 'Approvals' }, { id: 'trash' as const, label: 'Trash' }] : [{ id: 'appdata' as const, label: 'App data' }, { id: 'approvals' as const, label: 'Approvals' }]),
  ];
  ...
  return <main className="erp-app mx-auto max-w-6xl px-4 py-5 sm:px-6 sm:py-7">
    ...
    <div className="flex flex-col gap-5 lg:flex-row">
      <AppSidebar className="settings-sidebar w-full shrink-0 lg:w-56"><nav className="flex w-full gap-0 overflow-x-auto rounded-2xl border border-[#e6e2d6] bg-white p-0.5 shadow-sm lg:block lg:space-y-0.5">{navItems.map((item) => <button key={item.id} onClick={() => setActiveSection(item.id)} className={`flex min-h-8 shrink-0 items-center justify-between rounded-lg px-1.5 py-1 text-left text-[10px] font-bold transition lg:w-full ${activeSection === item.id ? 'bg-zinc-900 text-white' : 'text-zinc-600 hover:bg-[#f6f5ef]'}`}><span>{item.label}</span><ChevronRight className="hidden h-3 w-3 lg:block" /></button>)}</nav></AppSidebar>
