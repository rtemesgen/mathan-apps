...
function CompanyDeletionCard({ workspace, onNotice, onError, onRestored }: { workspace: { id: string; name: string }; onNotice: (value: string) => void; onError: (value: string) => void; onRestored: () => void }) {
  ...
  const [open, setOpen] = useState(false); const [confirmation, setConfirmation] = useState(''); const [password, setPassword] = useState(''); const [busy, setBusy] = useState(false);
  ...
  const schedule = async () => { if (confirmation !== `DELETE ${workspace.name}` || !password) return; setBusy(true); try { const { error: authError } = await supabase.auth.signInWithPassword({ email: (await supabase.auth.getUser()).data.user?.email ?? '', password }); if (authError) throw authError; const date = await requestWorkspaceDeletion(workspace.id); setOpen(false); setConfirmation(''); setPassword(''); onNotice(`Company scheduled for deletion on ${new Date(date).toLocaleDateString()}. You can restore it during the 30-day recovery period.`); await load(); onRestored(); } catch (reason) { onError(reason instanceof Error ? reason.message : 'Could not schedule company deletion.'); } finally { setBusy(false); } };
...
function PersonalDataCard({ userId, isOwner, onTransferCompany: onTransferCompanyRequest, onNotice, onError }: { userId?: string; isOwner: boolean; onTransferCompany: () => void; onNotice: (value: string) => void; onError: (value: string) => void }) {
  ...
  const exportData = async () => { if (!userId) return; setBusy(true); try { const [{ data: profile }, { data: memberships }, { data: snapshots }] = await Promise.all([supabase.from('workspace_profiles').select('user_id,display_name,phone,updated_at').eq('user_id', userId).maybeSingle(), supabase.rpc('list_my_workspaces'), supabase.from('app_state_snapshots').select('workspace_id,domain,payload,revision,updated_at')]); const archive = { format: 'mathan-personal-export', schema_version: '1', exported_at: new Date().toISOString(), profile, memberships, snapshots }; const blob = new Blob([JSON.stringify(archive, null, 2)], { type: 'application/json' }); const url = URL.createObjectURL(blob); const link = document.createElement('a'); link.href = url; link.download = `mathan-personal-data-${new Date().toISOString().slice(0, 10)}.json`; link.click(); URL.revokeObjectURL(url); onNotice('Personal data export downloaded.'); } catch (reason) { onError(reason instanceof Error ? reason.message : 'Could not export your data.'); } finally { setBusy(false); } };
