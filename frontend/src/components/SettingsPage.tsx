...
import { MyCompanyMemberships, WorkspaceInvitations } from './WorkspaceInvitations';
import { BackupRecoveryCard } from './BackupRecoveryCard';
import { isValidPhone, normalizePhone, PHONE_COUNTRIES } from '../auth/phone';
import { cancelAccountDeletion, cancelWorkspaceDeletion, getAccountDeletionRequest, getWorkspaceDeletionStatus, listWorkspaceTrash, purgeExpiredTrash, requestAccountDeletion, requestWorkspaceDeletion, restoreTrashRecord, type AccountDeletionRequest, type TrashRecord, type WorkspaceDeletionStatus } from '../lib/operationsApi';
...
const permissionLabels: Record<AppPermission, string> = { none: 'No access', view: 'View', edit: 'Edit' };

export function GuestSettingsPage() {
  const { workspace, signOut, renameGuestWorkspace } = useAuth();
  const [name, setName] = useState(workspace?.name ?? 'Guest Company');
  const [notice, setNotice] = useState('');
  useEffect(() => setName(workspace?.name ?? 'Guest Company'), [workspace?.id, workspace?.name]);
  const save = (event: FormEvent) => { event.preventDefault(); if (workspace && name.trim()) { renameGuestWorkspace(workspace.id, name); setNotice('Guest company saved on this device.'); } };
  const clear = async () => { if (!workspace || !window.confirm(`Clear all Cash Book, Payroll, and Truck Equity records for ${workspace.name}? This cannot be undone.`)) return; await clearGuestWorkspaceData(workspace.id); setNotice('Guest company records cleared from this device.'); };
  return <main className="erp-app mx-auto max-w-3xl px-4 py-6 sm:px-6"><div className="mb-5"><p className="text-[10px] font-extrabold uppercase tracking-[0.2em] text-emerald-700">Local guest settings</p><h1 className="mt-1 font-serif text-3xl font-bold">Guest mode</h1></div><section className="rounded-2xl border border-amber-200 bg-amber-50 p-4"><p className="text-sm font-bold text-amber-950">Everything in this guest company stays on this device.</p><p className="mt-1 text-xs leading-5 text-amber-800">Cash Book, Payroll, Truck Equity, reports, exports, and embedded attachments work offline. Log in when you want to choose a cloud company and import the data.</p><button type="button" onClick={() => void signOut()} className="mt-4 inline-flex items-center gap-2 rounded-xl bg-zinc-900 px-4 py-2.5 text-xs font-bold text-white"><LogIn className="h-4 w-4" /> Log in and sync</button></section>{notice && <p role="status" className="mt-4 rounded-xl bg-emerald-50 p-3 text-xs font-bold text-emerald-800">{notice}</p>}<form onSubmit={save} className="mt-4 rounded-2xl border border-[#e6e2d6] bg-white p-5 shadow-sm"><h2 className="font-serif text-xl font-bold">Local company</h2><label className="mt-4 block text-xs font-bold text-zinc-600">Company name<input required minLength={2} value={name} onChange={(event) => setName(event.target.value)} className="mt-1 w-full rounded-xl border p-3 text-sm" /></label><button className="mt-3 rounded-xl bg-zinc-900 px-4 py-2.5 text-xs font-bold text-white">Save locally</button></form><section className="mt-4 rounded-2xl border border-[#e6e2d6] bg-white p-5"><h2 className="font-serif text-xl font-bold">Available offline</h2><div className="mt-3 grid gap-2 sm:grid-cols-3">{(['book', 'payroll', 'truck'] as AppId[]).map((app) => <div key={app} className="rounded-xl bg-[#f6f5ef] p-3"><p className="text-sm font-bold">{appNames[app]}</p><p className="mt-1 text-[11px] text-emerald-700">Full local edit access</p></div>)}</div><p className="mt-4 text-xs text-zinc-500">Invitations, memberships, account security, notifications, and cloud deletion controls become available after login.</p></section><button type="button" onClick={() => void clear()} className="mt-4 inline-flex items-center gap-2 rounded-xl border border-red-200 bg-red-50 px-4 py-2.5 text-xs font-bold text-red-700"><Trash2 className="h-4 w-4" /> Clear this guest company’s data</button><AppVersionPanel /></main>;
}

function PermissionSelect({ value, onChange, disabled = false }: { value: AppPermission; onChange: (value: AppPermission) => void; disabled?: boolean }) {
  const [open, setOpen] = useState(false);
  return <div className="relative min-w-32">
    <button type="button" disabled={disabled} aria-haspopup="listbox" aria-expanded={open} onClick={() => setOpen((current) => !current)} className={`app-dropdown-trigger ${disabled ? 'cursor-not-allowed opacity-70' : ''}`}>
      <span className="truncate">{disabled ? 'Owner access' : permissionLabels[value]}</span>
      <ChevronDown className={`h-4 w-4 shrink-0 text-zinc-500 transition-transform ${open ? 'rotate-180' : ''}`} />
    </button>
    {open && !disabled && <div role="listbox" className="app-dropdown-menu right-0 min-w-full">
      {(Object.keys(permissionLabels) as AppPermission[]).map((permission) => <button key={permission} type="button" role="option" aria-selected={permission === value} onClick={() => { onChange(permission); setOpen(false); }} className={`app-dropdown-option ${permission === value ? 'app-dropdown-option-selected' : ''}`}>{permissionLabels[permission]}</button>)}
    </div>}
  </div>;
}
...
function OfflineCacheCard({ userId, online, onNotice, onError }: { userId?: string; online: boolean; onNotice: (value: string) => void; onError: (value: string) => void }) {
  ...
  const reset = async () => { if (!userId || !window.confirm('Clear downloaded company data from this device and download it again? Unsynchronized edits stay queued.')) return; setBusy(true); try { const count = await resetUserOfflineCache(userId); onNotice(`${count} cached records cleared. ${online ? 'Your companies will download again in the background.' : 'Reconnect to download them again.'}`); } catch (reason) { onError(reason instanceof Error ? reason.message : 'Could not reset offline cache.'); } finally { setBusy(false); } };
  return <section className="rounded-2xl border border-blue-200 bg-blue-50/60 p-5 shadow-sm"><div className="flex items-start justify-between gap-3"><div><h2 className="font-serif text-xl font-bold text-blue-950">Offline data</h2><p className="mt-1 text-xs leading-5 text-blue-900">Companies and app records are read from this device first. Cloud synchronization continues in the background when connected.</p></div><RefreshCw className="h-5 w-5 text-blue-700" /></div><p className="mt-3 text-[11px] font-semibold text-blue-800">{usage}</p><button type="button" disabled={busy} onClick={() => void reset()} className="mt-3 rounded-xl border border-blue-300 bg-white px-3 py-2 text-xs font-bold text-blue-900 disabled:opacity-50">{busy ? 'Resetting…' : 'Reset and download again'}</button></section>;
}
...
function CompanyDeletionCard({ workspace, onNotice, onError, onRestored }: { workspace: { id: string; name: string }; onNotice: (value: string) => void; onError: (value: string) => void; onRestored: () => void }) {
  const [status, setStatus] = useState<WorkspaceDeletionStatus>({ status: 'active', scheduled_for: null, days_remaining: null });
  const [open, setOpen] = useState(false); const [confirmation, setConfirmation] = useState(''); const [busy, setBusy] = useState(false);
  const load = async () => { try { setStatus(await getWorkspaceDeletionStatus(workspace.id)); } catch (reason) { onError(reason instanceof Error ? reason.message : 'Could not load company deletion status.'); } };
  useEffect(() => { void load(); }, [workspace.id]);
  const schedule = async () => { if (confirmation !== `DELETE ${workspace.name}`) return; setBusy(true); try { const date = await requestWorkspaceDeletion(workspace.id); setOpen(false); setConfirmation(''); onNotice(`Company scheduled for deletion on ${new Date(date).toLocaleDateString()}. You can restore it during the 30-day recovery period.`); await load(); onRestored(); } catch (reason) { onError(reason instanceof Error ? reason.message : 'Could not schedule company deletion.'); } finally { setBusy(false); } };
  const restore = async () => { setBusy(true); try { await cancelWorkspaceDeletion(workspace.id); onNotice('Company deletion cancelled. Your company is active again.'); await load(); onRestored(); } catch (reason) { onError(reason instanceof Error ? reason.message : 'Could not restore this company.'); } finally { setBusy(false); } };
  return <section className="rounded-2xl border border-red-200 bg-red-50/50 p-5 shadow-sm"><div className="flex items-start gap-3"><Trash2 className="mt-0.5 h-5 w-5 shrink-0 text-red-700" /><div className="min-w-0 flex-1"><h2 className="font-serif text-xl font-bold text-red-950">Company deletion</h2>{status.status === 'scheduled' ? <><p className="mt-1 text-xs leading-5 text-red-900">This company is scheduled for deletion. It will be permanently removed in {status.days_remaining ?? 0} day{status.days_remaining === 1 ? '' : 's'}.</p><button type="button" disabled={busy} onClick={() => void restore()} className="mt-4 rounded-xl bg-emerald-700 px-4 py-2 text-xs font-bold text-white disabled:opacity-50">{busy ? 'Restoring…' : 'Restore company'}</button></> : <><p className="mt-1 text-xs leading-5 text-red-900">Only the owner can schedule deletion. Records and files remain recoverable for 30 days before permanent cleanup.</p><button type="button" onClick={() => setOpen(true)} className="mt-4 rounded-xl border border-red-300 bg-white px-4 py-2 text-xs font-bold text-red-800 hover:bg-red-100">Schedule company deletion</button></>}</div></div>{open && <AppDialog open={open} title="Schedule company deletion" onClose={() => { setOpen(false); setConfirmation(''); }} footer={<><AppButton type="button" onClick={() => setOpen(false)}>Cancel</AppButton><AppButton type="button" variant="danger" disabled={busy || confirmation !== `DELETE ${workspace.name}`} onClick={() => void schedule()}>{busy ? 'Scheduling…' : 'Schedule deletion'}</AppButton></>}><p className="text-sm leading-6 text-zinc-600">All members, apps, records, and attachments will become unavailable and be permanently removed after 30 days. You can restore the company before then.</p><label className="mt-4 block text-xs font-bold text-zinc-700">Type <span className="font-mono">DELETE {workspace.name}</span> to continue<input autoFocus value={confirmation} onChange={(event) => setConfirmation(event.target.value)} placeholder={`DELETE ${workspace.name}`} className="mt-2 w-full rounded-xl border border-[#e6e2d6] bg-white px-3 py-2 text-sm" /></label></AppDialog>}</section>;
}
...
function TrashCard({ workspaceId, onNotice, onError }: { workspaceId?: string; onNotice: (value: string) => void; onError: (value: string) => void }) {
  const [items, setItems] = useState<TrashRecord[]>([]); const [busy, setBusy] = useState('');
  const load = async () => { if (!workspaceId) return; try { setItems(await listWorkspaceTrash(workspaceId)); } catch (reason) { onError(reason instanceof Error ? reason.message : 'Could not load trash.'); } };
  useEffect(() => { void load(); }, [workspaceId]);
  const restore = async (item: TrashRecord) => { setBusy(item.record_id); try { await restoreTrashRecord(item.source_table, item.record_id); onNotice(`${item.label} restored.`); await load(); } catch (reason) { onError(reason instanceof Error ? reason.message : 'Could not restore record.'); } finally { setBusy(''); } };
  const purge = async () => { if (!workspaceId) return; setBusy('purge'); try { const count = await purgeExpiredTrash(workspaceId); onNotice(`${count} expired record${count === 1 ? '' : 's'} permanently removed.`); await load(); } catch (reason) { onError(reason instanceof Error ? reason.message : 'Could not clean up expired records.'); } finally { setBusy(''); } };
  return <section className="rounded-2xl border border-[#e6e2d6] bg-white p-5 shadow-sm"><div className="flex items-start justify-between gap-3"><div><h2 className="font-serif text-xl font-bold">Trash and recovery</h2><p className="mt-1 text-xs leading-5 text-zinc-500">Deleted records remain recoverable for 30 days before permanent cleanup.</p></div><Trash2 className="h-5 w-5 text-amber-700" /></div><div className="mt-4 space-y-2">{items.length ? items.map((item) => <div key={`${item.source_table}:${item.record_id}`} className="flex items-center gap-3 rounded-xl bg-[#faf9f5] p-3"><div className="min-w-0 flex-1"><p className="truncate text-xs font-bold">{item.label || item.source_table}</p><p className="text-[10px] text-zinc-500">{item.source_table} · deleted {new Date(item.deleted_at).toLocaleDateString()}</p></div><button type="button" disabled={busy === item.record_id} onClick={() => void restore(item)} className="inline-flex items-center gap-1 rounded-lg border border-emerald-200 px-2.5 py-1.5 text-[10px] font-bold text-emerald-800"><RefreshCw className="h-3 w-3" /> Restore</button></div>) : <p className="text-xs text-zinc-500">Trash is empty.</p>}</div><button type="button" disabled={busy === 'purge'} onClick={() => void purge()} className="mt-4 rounded-xl border border-red-200 px-3 py-2 text-xs font-bold text-red-700 disabled:opacity-50">Clean up expired records</button></section>;
}

function PersonalDataCard({ userId, isOwner, onTransferCompany: onTransferCompanyRequest, onNotice, onError }: { userId?: string; isOwner: boolean; onTransferCompany: () => void; onNotice: (value: string) => void; onError: (value: string) => void }) {
  const [request, setRequest] = useState<AccountDeletionRequest | null>(null); const [busy, setBusy] = useState(false); const [choiceOpen, setChoiceOpen] = useState(false); const [confirmOpen, setConfirmOpen] = useState(false); const [deleteCompanies, setDeleteCompanies] = useState(false);
  const onTransferCompany = () => { if (isOwner) onTransferCompanyRequest(); else onError('You do not own a company that needs an ownership transfer.'); };
  useEffect(() => { if (userId) void getAccountDeletionRequest(userId).then(setRequest).catch((reason) => onError(reason instanceof Error ? reason.message : 'Could not load account status.')); }, [userId]);
  const exportData = async () => { if (!userId) return; setBusy(true); try { const [{ data: profile }, { data: memberships }, { data: snapshots }, { data: trucks }, { data: truckOwners }, { data: truckTransactions }] = await Promise.all([supabase.from('workspace_profiles').select('user_id,display_name,phone,updated_at').eq('user_id', userId).maybeSingle(), supabase.rpc('list_my_workspaces'), supabase.from('app_state_snapshots').select('workspace_id,domain,payload,revision,updated_at'), supabase.from('trucks').select('*'), supabase.from('truck_owners').select('*'), supabase.from('truck_transactions').select('*')]); const archive = { format: 'mathan-personal-export', schema_version: '1', exported_at: new Date().toISOString(), profile, memberships, snapshots, trucks, truck_owners: truckOwners, truck_transactions: truckTransactions }; const blob = new Blob([JSON.stringify(archive, null, 2)], { type: 'application/json' }); const url = URL.createObjectURL(blob); const link = document.createElement('a'); link.href = url; link.download = `mathan-personal-data-${new Date().toISOString().slice(0, 10)}.json`; link.click(); URL.revokeObjectURL(url); onNotice('Personal data export downloaded.'); } catch (reason) { onError(reason instanceof Error ? reason.message : 'Could not export your data.'); } finally { setBusy(false); } };
  const requestDelete = async () => { setConfirmOpen(false); setBusy(true); try { setRequest(await requestAccountDeletion(deleteCompanies)); onNotice(deleteCompanies ? 'Account and solely owned companies scheduled for deletion. You can cancel during the 30-day recovery period.' : 'Account deletion requested. Transfer ownership first to preserve company data.'); } catch (reason) { onError(reason instanceof Error ? reason.message : 'Could not request account deletion.'); } finally { setBusy(false); } };
  const cancelDelete = async () => { setBusy(true); try { await cancelAccountDeletion(); setRequest(null); onNotice('Account deletion cancelled.'); } catch (reason) { onError(reason instanceof Error ? reason.message : 'Could not cancel account deletion.'); } finally { setBusy(false); } };
  return <><section className="rounded-2xl border border-[#e6e2d6] bg-white p-5 shadow-sm"><h2 className="font-serif text-xl font-bold">Your data and account</h2><p className="mt-1 text-xs leading-5 text-zinc-500">Download a copy of your profile, memberships, and synced application data.</p><div className="mt-4 flex flex-wrap gap-2"><button type="button" disabled={busy} onClick={() => void exportData()} className="rounded-xl bg-zinc-900 px-3 py-2 text-xs font-bold text-white">Download personal data</button>{request?.status === 'pending' ? <button type="button" disabled={busy} onClick={() => void cancelDelete()} className="rounded-xl border border-emerald-200 px-3 py-2 text-xs font-bold text-emerald-800">Cancel deletion · due {new Date(request.scheduled_for).toLocaleDateString()}</button> : <button type="button" disabled={busy} onClick={() => setChoiceOpen(true)} className="rounded-xl border border-red-200 px-3 py-2 text-xs font-bold text-red-700">Delete account</button>}</div></section>{choiceOpen && <div className="fixed inset-0 z-50 flex items-center justify-center bg-zinc-950/50 p-4"><section role="dialog" aria-modal="true" className="w-full max-w-md rounded-3xl border border-[#e6e2d6] bg-white p-5 shadow-2xl"><h2 className="font-serif text-xl font-bold">How should we handle your companies?</h2><p className="mt-2 text-xs leading-5 text-zinc-500">Choose whether to preserve company data by transferring ownership or delete companies that you solely own with your account.</p><div className="mt-5 grid gap-2"><button type="button" onClick={() => { setChoiceOpen(false); setDeleteCompanies(true); setConfirmOpen(true); }} className="rounded-xl bg-red-700 p-3 text-left text-xs font-bold text-white"><span className="block">Delete everything</span><span className="mt-1 block text-[10px] font-normal text-red-100">Schedule your account and solely owned companies for deletion.</span></button><button type="button" onClick={() => { setChoiceOpen(false); onTransferCompany(); }} className="rounded-xl border border-amber-200 bg-amber-50 p-3 text-left text-xs font-bold text-amber-900"><span className="block">Transfer company ownership</span><span className="mt-1 block text-[10px] font-normal text-amber-800">Choose a company member to keep each company and its data.</span></button><button type="button" onClick={() => setChoiceOpen(false)} className="rounded-xl border border-[#e6e2d6] p-3 text-xs font-bold text-zinc-700">Cancel</button></div></section></div>}<DeleteConfirmModal isOpen={confirmOpen} title="Delete account and companies?" message="This schedules your account and solely owned companies for permanent deletion after the 30-day recovery period. This cannot be undone after that date." onClose={() => setConfirmOpen(false)} onConfirm={() => void requestDelete()} confirmLabel="Schedule deletion" /></>;
}

function OwnershipTransferCard({ members, busy, onTransfer }: { members: Member[]; busy: boolean; onTransfer: (id: string) => void }) {
  const [target, setTarget] = useState('');
  const candidates = members.filter((member) => member.role === 'member');
  if (!candidates.length) return null;
  return <section className="rounded-2xl border border-amber-200 bg-amber-50 p-5 shadow-sm"><h2 className="font-serif text-xl font-bold text-amber-950">Transfer company ownership</h2><p className="mt-1 text-xs leading-5 text-amber-900">Choose an existing member to become the owner. You will become a regular member and can then request account deletion.</p><div className="mt-4 flex flex-wrap gap-2"><AppSelect value={target} onChange={setTarget} placeholder="Choose a member" options={candidates.map((member) => ({ value: member.user_id, label: member.display_name || member.email }))} className="min-w-0 flex-1" /><button type="button" disabled={!target || busy} onClick={() => { onTransfer(target); setTarget(''); }} className="rounded-xl bg-amber-800 px-3 py-2 text-xs font-bold text-white disabled:opacity-40">Transfer ownership</button></div></section>;
}
...
export function SettingsPage() {
  ...
  const [invitePermissions, setInvitePermissions] = useState<Record<AppId, AppPermission>>({ book: 'none', payroll: 'none', truck: 'none' });
  const [notice, setNotice] = useState('');
  const [error, setError] = useState('');
  const [busy, setBusy] = useState(false);
  const [inviteLink, setInviteLink] = useState('');
  const requestedSection = new URLSearchParams(window.location.search).get('section');
  type SettingsSection = 'company' | 'apps' | 'invites' | 'people' | 'profile' | 'security' | 'approvals' | 'trash';
  const settingsSectionKey = `mathan_settings_section_${user?.id ?? 'guest'}_${workspace?.id ?? 'none'}`;
  const storedSection = (() => { try { return localStorage.getItem(settingsSectionKey) as SettingsSection | null; } catch { return null; } })();
  const [activeSection, setActiveSection] = useState<SettingsSection>(requestedSection === 'invites' ? 'invites' : storedSection ?? 'profile');
  const [expandedMember, setExpandedMember] = useState<string | null>(null);
  const [memberToRemove, setMemberToRemove] = useState<Member | null>(null);
  const [companyAccess, setCompanyAccess] = useState<Record<string, CompanyAccess[]>>({});
  const [appToDisable, setAppToDisable] = useState<AppId | null>(null);
  const requireOnline = () => { if (online) return true; setError('This settings change needs an internet connection. Your company records remain available offline.'); return false; };
  useEffect(() => { try { localStorage.setItem(settingsSectionKey, activeSection); } catch { /* storage may be unavailable */ } }, [settingsSectionKey, activeSection]);
  ...
  const navItems = [
    ...
    ...(isOwner ? [{ id: 'company' as const, label: 'Company' }, { id: 'apps' as const, label: 'Apps' }, { id: 'invites' as const, label: 'Invitations' }, { id: 'approvals' as const, label: 'Approvals' }, { id: 'trash' as const, label: 'Trash' }] : [{ id: 'approvals' as const, label: 'Approvals' }]),
  ];
  const sectionTitle = navItems.find((item) => item.id === activeSection)?.label ?? 'Settings';

  return <main className="erp-app mx-auto max-w-6xl px-4 py-5 sm:px-6 sm:py-7">
    <div className="mb-5 flex items-start gap-3"><ShieldCheck className="mt-1 h-6 w-6 text-emerald-700" /><div><p className="text-[10px] font-extrabold uppercase tracking-[0.2em] text-emerald-700">Workspace settings</p><h1 className="mt-1 font-serif text-3xl font-bold text-zinc-900">{sectionTitle}</h1></div></div>
    ...
    {!online && <div className="mb-4 rounded-xl border border-amber-200 bg-amber-50 p-3 text-xs font-semibold text-amber-900">Offline · company records can be viewed and edited, but account, member, and company settings require a connection.</div>}
    ...
    <div className="flex flex-col gap-5 lg:flex-row">
      ...
      <div className="min-w-0 flex flex-1 flex-col">
        ...
        {activeSection === 'profile' && <div className="order-1 space-y-4"><WorkspaceInvitations /><OfflineCacheCard userId={user?.id} online={online} onNotice={setNotice} onError={setError} /><section className="rounded-2xl border border-emerald-200 bg-emerald-50 p-4"><h2 className="font-serif text-lg font-bold text-emerald-950">Guest data</h2><p className="mt-1 text-xs leading-5 text-emerald-800">If this device has local guest companies, you can merge them into an eligible cloud company without replacing its existing records.</p><button type="button" onClick={() => { sessionStorage.removeItem('mathan_guest_import_deferred'); window.location.href = '/'; }} className="mt-3 rounded-xl bg-emerald-800 px-4 py-2.5 text-xs font-bold text-white">Check for guest data</button></section><SecuritySettingsCard password={password} editing={editingSecurity} showPassword={showPassword} busy={busy} onEdit={() => setEditingSecurity(true)} onCancel={() => { setPassword(''); setShowPassword(false); setEditingSecurity(false); }} onSubmit={updatePassword} onTogglePassword={() => setShowPassword((current) => !current)} setPassword={setPassword} /><SessionSecurityCard onNotice={setNotice} onError={setError} /><BackupRecoveryCard /><PersonalDataCard userId={user?.id} isOwner={isOwner} onTransferCompany={() => setActiveSection('company')} onNotice={setNotice} onError={setError} /><MyCompanyMemberships /></div>}
        ...
        {activeSection === 'apps' && isOwner && <section className="space-y-3">{(['book', 'payroll', 'truck'] as AppId[]).map((app) => <div key={app} className="flex items-center justify-between rounded-2xl border border-[#e6e2d6] bg-white p-4 shadow-sm"><div className="flex items-center gap-3"><span className="flex h-9 w-9 items-center justify-center rounded-xl bg-emerald-50 text-emerald-700"><Power className="h-4 w-4" /></span><div><p className="text-sm font-bold">{appNames[app]}</p><p className="text-xs text-zinc-500">{appAccess[app].enabled ? 'Available to permitted members' : 'Disabled and hidden'}</p></div></div><div className="flex items-center gap-3"><span className="text-[10px] font-extrabold uppercase tracking-wider text-zinc-500">{appAccess[app].enabled ? 'On' : 'Off'}</span><AppToggle enabled={appAccess[app].enabled} onChange={() => { if (appAccess[app].enabled) setAppToDisable(app); else void toggleApp(app); }} /></div></div>)}</section>}
        {activeSection === 'invites' && isOwner && <section className="rounded-2xl border border-[#e6e2d6] bg-white p-5 shadow-sm"><div className="flex items-start justify-between"><div><h2 className="font-serif text-xl font-bold">Invite people</h2><p className="mt-1 text-xs text-zinc-500">Choose access before creating a secure seven-day invitation.</p></div><UserPlus className="h-5 w-5 text-emerald-700" /></div><form onSubmit={createInvite} className="mt-4 max-w-2xl space-y-3"><input required type="email" value={inviteEmail} onChange={(event) => setInviteEmail(event.target.value)} placeholder="person@company.com" className="w-full rounded-xl border border-[#e6e2d6] px-3 py-2 text-sm" /><div className="grid gap-2 sm:grid-cols-3"><label className="text-[11px] font-bold text-zinc-500">Cash Book<PermissionSelect value={invitePermissions.book} onChange={(value) => setInvitePermissions((current) => ({ ...current, book: value }))} /></label><label className="text-[11px] font-bold text-zinc-500">Payroll<PermissionSelect value={invitePermissions.payroll} onChange={(value) => setInvitePermissions((current) => ({ ...current, payroll: value }))} /></label><label className="text-[11px] font-bold text-zinc-500">Truck Equity<PermissionSelect value={invitePermissions.truck} onChange={(value) => setInvitePermissions((current) => ({ ...current, truck: value }))} /></label></div><button disabled={busy} className="inline-flex items-center justify-center gap-2 rounded-xl bg-zinc-900 px-4 py-2 text-xs font-bold text-white"><Mail className="h-3.5 w-3.5" /> Create invite</button></form>{inviteLink && <div className="mt-4 flex flex-wrap gap-2 rounded-xl bg-emerald-50 p-2"><input readOnly value={inviteLink} className="min-w-[220px] flex-1 bg-transparent px-2 text-xs text-emerald-900" /><button onClick={() => void copyLink()} className="inline-flex items-center gap-1 rounded-lg bg-white px-2 py-1 text-xs font-bold text-emerald-800"><Copy className="h-3 w-3" /> Copy</button><button onClick={() => void shareCurrentInvite()} className="inline-flex items-center gap-1 rounded-lg bg-emerald-700 px-2 py-1 text-xs font-bold text-white"><Share2 className="h-3 w-3" /> Share invite</button></div>}{pendingInvitations.length > 0 && <div className="mt-7 border-t border-[#e6e2d6] pt-4"><p className="text-xs font-extrabold uppercase tracking-wider text-zinc-500">Pending invitations</p>{pendingInvitations.map((invitation) => <div key={invitation.id} className="mt-2 flex items-center justify-between rounded-xl border border-dashed border-zinc-300 p-3"><div><p className="text-sm font-semibold">{invitation.email}</p><p className="text-[11px] text-zinc-500">Cash Book: {permissionLabels[invitation.book_permission]} · Payroll: {permissionLabels[invitation.payroll_permission]} · Truck Equity: {permissionLabels[invitation.truck_permission]} · Expires {new Date(invitation.expires_at).toLocaleDateString()}</p></div><button onClick={() => void revokeInvite(invitation.id)} className="rounded-lg p-2 text-zinc-500 hover:bg-zinc-100"><X className="h-4 w-4" /></button></div>)}</div>}</section>}
        ...
        {activeSection === 'people' && isOwner && <section className="mt-3 rounded-2xl border border-amber-200 bg-amber-50/60 p-4 shadow-sm"><h2 className="font-serif text-lg font-bold">Truck Equity access</h2><p className="mt-1 text-xs text-zinc-600">Set each member’s Truck permission independently.</p><div className="mt-3 space-y-2">{members.map((member) => <div key={member.user_id} className="flex flex-wrap items-center justify-between gap-2 rounded-xl bg-white p-3"><div className="min-w-0"><p className="truncate text-xs font-bold">{member.display_name || member.email}</p><p className="truncate text-[10px] text-zinc-500">{member.email} · {member.role}</p></div><PermissionSelect value={member.truck_permission ?? 'none'} disabled={member.role === 'owner'} onChange={(permission) => void updateMemberPermission(member, 'truck', permission)} /></div>)}</div></section>}
