...
import { Check, ChevronDown, ChevronRight, Copy, Mail, Power, Share2, ShieldCheck, UserMinus, UserPlus, X } from 'lucide-react';
...
export function SettingsPage() {
  ...
  const [activeSection, setActiveSection] = useState<'company' | 'apps' | 'invites' | 'people' | 'profile' | 'security'>(isOwner ? 'company' : 'profile');
  ...
  const navItems = [
    ...(isOwner ? [{ id: 'company' as const, label: 'Company' }, { id: 'apps' as const, label: 'Apps' }, { id: 'invites' as const, label: 'Invitations' }, { id: 'people' as const, label: 'People and access' }] : []),
    { id: 'profile' as const, label: 'Your profile' }, { id: 'security' as const, label: 'Security' },
  ];
