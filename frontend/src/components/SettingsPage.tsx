...
function OwnershipTransferCard({ members, busy, onTransfer }: { members: Member[]; busy: boolean; onTransfer: (id: string) => void }) {
  ...
  return <section className="rounded-2xl border border-amber-200 bg-amber-50 p-5 shadow-sm"><h2 className="font-serif text-xl font-bold text-amber-950">Transfer company ownership</h2><p className="mt-1 text-xs leading-5 text-amber-900">Choose an existing member to become the owner. You will become a regular member and can then request account deletion.</p><div className="mt-4 flex flex-wrap gap-2"><select value={target} onChange={(event) => setTarget(event.target.value)} className="min-w-0 flex-1 rounded-xl border border-amber-200 bg-white px-3 py-2 text-xs"><option value="">Choose a member</option>{candidates.map((member) => <option key={member.user_id} value={member.user_id}>{member.display_name || member.email}</option>)}</select><button type="button" disabled={!target || busy} onClick={() => { onTransfer(target); setTarget(''); }} className="rounded-xl bg-amber-800 px-3 py-2 text-xs font-bold text-white disabled:opacity-40">Transfer ownership</button></div></section>;
}
