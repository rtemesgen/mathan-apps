...
export function CompanySelector() {
  const { workspace, workspaces, switchWorkspace, refreshWorkspace, signOut } = useAuth();
  ...
  const createCompany = async (event: FormEvent) => {
    ...
    if (userError || !userCheck.user) { await signOut(); setError('Your sign-in session is no longer valid. Please sign in again.'); setBusy(false); return; }
    const { data, error: createError } = await supabase.rpc('create_workspace', { workspace_name: cleaned });
    if (createError) { setError(createError.code === '23503' || createError.message.toLowerCase().includes('created_by_fkey') ? 'Your sign-in session is no longer valid. Please sign in again.' : createError.message); setBusy(false); return; }
  ...
  return <><main className="mx-auto max-w-4xl px-4 py-7 sm:px-6 sm:py-10"><div className="mb-7 flex items-start justify-between gap-4"><div><p className="text-[10px] font-extrabold uppercase tracking-[0.2em] text-emerald-700">Your companies</p><h1 className="mt-1 font-serif text-3xl font-bold">Choose a company</h1><p className="mt-2 text-sm text-zinc-500">Each company has its own apps, records, and permissions.</p></div><Building2 className="h-7 w-7 text-emerald-700" /></div>
    ...
    <section className="mt-7 rounded-2xl border border-dashed border-[#cfcabb] bg-[#faf9f5] p-4"><button onClick={() => setShowCreate((value) => !value)} className="inline-flex items-center gap-2 text-sm font-bold text-zinc-900"><Plus className="h-4 w-4 text-emerald-700" /> Create another company</button>{showCreate && <form onSubmit={createCompany} className="mt-4 flex flex-col gap-2 sm:flex-row"><input autoFocus required minLength={2} value={name} onChange={(event) => setName(event.target.value)} placeholder="Company name" className="min-w-0 flex-1 rounded-xl border border-[#e6e2d6] bg-white px-3 py-2 text-sm" /><button disabled={busy} className="rounded-xl bg-zinc-900 px-4 py-2 text-xs font-bold text-white">{busy ? 'Creating…' : 'Create company'}</button></form>}{error && <p className="mt-2 text-xs font-semibold text-red-700">{error}</p>}</section>
