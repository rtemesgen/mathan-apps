...
import { ArrowRight, Building2, Check, Plus, ShieldCheck } from 'lucide-react';
...
export function CompanySelector() {
  ...
  const owners = workspaces.filter((item) => item.role === 'owner');
  ...
  const selectCompany = (id: string) => {
    ...
    if (!next) return;
    if (!window.confirm(`Switch to ${next.name}? Your current company view will close.`)) return;
    switchWorkspace(id);
    navigate('/');
  };
