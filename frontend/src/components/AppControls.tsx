...
export function AppControls() {
  ...
  useEffect(() => {
    if (!user || standaloneMode || isGuest) { setInvitationCount(0); return; }
    const load = async () => { const { data } = await supabase.rpc('count_my_workspace_invitations'); setInvitationCount(Number(data ?? 0)); };
    void load();
    const onRefresh = () => void load();
    window.addEventListener('mathan:invitations-changed', onRefresh);
    window.addEventListener('focus', onRefresh);
    return () => { window.removeEventListener('mathan:invitations-changed', onRefresh); window.removeEventListener('focus', onRefresh); };
  }, [user?.id, isGuest]);
  return (
    <div className="flex items-center gap-1.5 sm:gap-2">
      ...
      {!standaloneMode && user && <Link to="/settings?section=invites" aria-label={`${invitationCount} pending invitations`} className="relative inline-flex h-8 w-8 items-center justify-center rounded-lg border border-[#e6e2d6] bg-white text-zinc-700 shadow-sm hover:bg-[#faf9f5]"><Bell className="h-4 w-4" />{invitationCount > 0 && <span className="absolute -right-1 -top-1 flex h-4 min-w-4 items-center justify-center rounded-full bg-red-600 px-1 text-[9px] font-extrabold text-white">{invitationCount > 9 ? '9+' : invitationCount}</span>}</Link>}
