...
function recordLabel(event: WorkspaceAuditEvent) {
  const domain = typeof event.next_data?.domain === 'string'
    ? event.next_data.domain
    : typeof event.previous_data?.domain === 'string'
      ? event.previous_data.domain
      : event.record_type;
  if (domain.startsWith('cash_book:')) return `Cash Book · ${domain.slice('cash_book:'.length)}`;
  if (domain.startsWith('payroll:')) return `Payroll · ${domain.slice('payroll:'.length)}`;
  if (domain === 'cash_book') return 'Cash Book';
  if (domain === 'payroll') return 'Payroll';
  return domain;
}
