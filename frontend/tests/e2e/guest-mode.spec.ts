...
test('guest Cash Book, Payroll, and Truck data merge idempotently after login', async ({ page }) => {
  ...
  const fixture = await page.evaluate(async () => {
    ...
    const database = await new Promise<IDBDatabase>((resolve, reject) => { const request = indexedDB.open('mathan-erp-offline'); request.onupgradeneeded = () => request.result.createObjectStore('records'); request.onsuccess = () => resolve(request.result); request.onerror = () => reject(request.error); });
