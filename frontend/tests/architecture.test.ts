...
assert.match(read('apps/book/components/BookDetailView.tsx'), /onEditTransaction/);
assert.match(read('apps/book/components/BookDetailView.tsx'), /MoreVertical/);
assert.match(read('apps/book/components/BookDetailView.tsx'), /role="menuitem"/);
assert.match(read('apps/book/components/TransactionModal.tsx'), /initialTransaction/);
assert.match(read('apps/book/components/TransactionModal.tsx'), /Update Entry/);
assert.match(read('apps/book/cashBookRepository.ts'), /updateTransaction/);
...
assert.match(read('lib/uuid.ts'), /export function createUuid/);
assert.match(read('lib/localStore.ts'), /openIndexedDbAtSafeVersion/);
assert.match(read('lib/indexedDbVersion.ts'), /safeIndexedDbVersion/);
assert.doesNotMatch([...collectTsx('apps'), read('auth/guestWorkspaces.ts'), read('auth/AuthProvider.tsx'), read('lib/localStore.ts')].join('\n'), /crypto\.randomUUID\(\)/, 'production code must use the LAN-safe UUID helper');
...
assert.match(read('apps/payroll/views/AddEmployeeView.tsx'), /createUuid\(\)/, 'offline employees must use permanent UUIDs');
assert.match(read('apps/payroll/views/ManageEmployeesView.tsx'), /createUuid\(\)/, 'offline salary changes must use permanent UUIDs');
assert.match(read('apps/payroll/views/AddRaiseView.tsx'), /createUuid\(\)/, 'offline raises must use permanent UUIDs');
assert.match(read('apps/payroll/views/PaySalaryView.tsx'), /createUuid\(\)/, 'offline payroll payments must use permanent UUIDs');
