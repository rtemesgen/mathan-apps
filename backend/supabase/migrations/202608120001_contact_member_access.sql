...
begin
  if not public.is_workspace_owner(target_workspace) then
    raise exception 'Only workspace owners can look up contacts';
  end if;
  return query
  select u.phone,
         u.id,
         coalesce(p.display_name, u.raw_user_meta_data ->> 'name', u.email, u.phone),
         exists (select 1 from public.workspace_members wm where wm.workspace_id = target_workspace and wm.user_id = u.id)
  from auth.users u
  left join public.workspace_profiles p on p.user_id = u.id
  where u.phone is not null
    and regexp_replace(u.phone, '[^0-9]', '', 'g') in (
      select regexp_replace(value, '[^0-9]', '', 'g') from unnest(target_phones) as value
    );
end;
$$;
...
begin
  ...
  where phone is not null and regexp_replace(phone, '[^0-9]', '', 'g') = regexp_replace(target_phone, '[^0-9]', '', 'g')
  limit 1;
