-- App permissions are explicit for every workspace member, including owners.
-- Existing owners are seeded with `edit`; changing a permission in Settings
-- must therefore affect the actual authorization checks as well as the UI.
...
returns boolean language sql stable security definer set search_path = public as $$
  select public.workspace_app_enabled(target_workspace, target_app) and exists (
    ...
    where m.workspace_id = target_workspace
      and m.user_id = auth.uid()
      and coalesce(p.permission, 'none') = 'edit'
  );
$$;
...
language sql security definer set search_path = public as $$
  select w.id, w.name, w.accent_color, m.role,
    public.workspace_app_enabled(w.id, 'book'),
    coalesce(pb.permission, 'none'::public.app_permission),
    public.workspace_app_enabled(w.id, 'payroll'),
    coalesce(pp.permission, 'none'::public.app_permission)
  from public.workspace_members m
  join public.workspaces w on w.id = m.workspace_id
  left join public.workspace_member_app_permissions pb
    on pb.workspace_id = m.workspace_id and pb.user_id = m.user_id and pb.app_id = 'book'
  left join public.workspace_member_app_permissions pp
    on pp.workspace_id = m.workspace_id and pp.user_id = m.user_id and pp.app_id = 'payroll'
