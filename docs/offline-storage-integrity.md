...
- The 5 MB attachment policy has unit coverage. Physical API 30 instrumentation also measured 1 MB, 3 MB and 4.9 MB single attachments plus a 4.9 MB record split across three attachments with two queued copies; broader queue-depth, peak-memory, signed-release and 16 KB runtime evidence is still required before raising or removing that bound.
