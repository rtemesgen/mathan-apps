...
5. Android instrumentation covers encrypted local writes, restart reads, interrupted migration recovery, failed writes, and force-stop queue recovery. The normal Android job also carries a real offline Truck mutation through host-driven process death and server acknowledgement; the constrained 16 KB job keeps that backend assertion skipped because its WebView/runtime gate is intentionally SQLite-focused. Backend-connected Android and physical-device capacity/page-size evidence is supplied by CI/release runs, not inferred from this document.
