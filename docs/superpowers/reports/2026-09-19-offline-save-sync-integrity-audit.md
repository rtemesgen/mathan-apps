...
- Full elevated browser run on the current branch: 25/28 passed. The three failures occurred during the long sequential run: both restart regressions passed when rerun in isolation, and one failure reported the local Supabase database container becoming unhealthy. The combined multimodule flow and legacy split-Payroll migration/restart also pass in isolation after the reconnect test synchronization correction.
- Focused verification after the harness corrections: `npm run test:unit`, `npm run lint`, `npm run test:snapshot-sync`, `npm run test:snapshot-save`, `npm run test:truck`, `npm run build`, the affected Cash Book/Payroll regressions (2/2), the combined multimodule flow (1/1), and the legacy split-Payroll migration/restart flow (1/1) — passed. The dedicated Truck restart and customer-projection scenarios pass; the ordinary Payroll restart scenario passes.
