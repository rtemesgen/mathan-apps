...
- External CI evidence: run `35443326123` passed frontend, database, and browser E2E jobs but failed Android instrumentation because the force-stop relaunch race crashed `staleQueueRecoversAfterPendingSaveAndForceStopBoundary`; run `35443807254` tests the `2dfd953` fix and was still in progress at this checkpoint.
