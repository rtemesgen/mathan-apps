...
- Full elevated browser run on the current branch: 25/28 passed. The three failures occurred during the long sequential run: two restart regressions passed when rerun in isolation, and one failure reported the local Supabase database container becoming unhealthy. The combined multimodule flow also passed in isolation after the test now waits for the authenticated Cash Book UI before replaying the reconnect event.
- Focused verification after the harness corrections: `npm run test:unit`, `npm run lint`, `npm run test:snapshot-sync`, `npm run test:snapshot-save`, `npm run test:truck`, `npm run build`, the two affected Cash Book/Payroll regressions (2/2), and the combined multimodule flow (1/1) — passed. The dedicated Truck restart and customer-projection scenarios pass; the ordinary Payroll restart scenario passes. The legacy split-Payroll restart remains unverified because its cold local run exceeded 180 seconds.
