...
**Status: implementation in progress; audited checkpoint on `fix-andriod` (2026-09-20).** This revision incorporates the user's detailed requirements supplied after the review. The branch contains implemented and tested portions of phases 0A–8, including native coordination, queue v2 metadata, atomic snapshot acknowledgement, transactional Truck batches, conflict-path wiring, top-level snapshot-array reconciliation, CI-backed browser/database checks, and backend-connected browser tests for both snapshot and Truck conflict choices. Normal-page physical Android runtime evidence now passes on `SM-N971N` (API 30), the debug previous-build→current-build replacement test preserved durable data, the physical SQLite capacity baseline now covers one attachment at 1 MB, 3 MB and 4.9 MB plus a 4.9 MB record split across three attachments with two queued copies, and the required CI 16 KB emulator runtime suite covers the SQLite/WebView smoke path. The constrained 16 KB emulator intentionally skips the 4.9 MB bridge stress matrix; that full capacity matrix remains a normal-runtime/physical-device gate. The plan remains open: final signed-release/physical 16 KB evidence, the full browser race matrix, signed-release APK replacement, broader large-queue/capacity measurements, native reproduction of the original phone error, and release/pilot gates are not yet proven. Editing this document does not authorize production database deployment or data cleanup.

**Current branch-tip evidence:** `0a932436` is clean and synchronized with `origin/fix-andriod`. `npm test` passes locally (the SQLite adapter test requires permission to launch the `sqlite3` child process), and CI run [35507042934](https://github.com/rtemesgen/mathan-apps/actions/runs/35507042934) reports success for frontend, database, browser E2E, normal Android, Android 16 KB runtime, and the required-gates aggregate. The 16 KB job reports a 16,384-byte page size and completes the instrumentation suite after CI-only background-service/memory stabilization. This closes the required emulator-CI gate, not final signed-release or physical-device evidence.
