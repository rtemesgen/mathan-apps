...
**Status: implementation in progress; audited checkpoint on `fix-andriod` (2026-09-20).** This revision incorporates the user's detailed requirements supplied after the review. The branch contains implemented and tested portions of phases 0A–8, including native coordination, queue v2 metadata, atomic snapshot acknowledgement, transactional Truck batches, conflict-path wiring, top-level snapshot-array reconciliation, CI-backed browser/database checks, backend-connected browser tests for both snapshot and Truck conflict choices, and a host-driven Android process-death path that now carries a real offline Truck mutation to post-restart server acknowledgement in the normal Android job. Normal-page physical Android runtime evidence now passes on `SM-N971N` (API 30), the debug previous-build→current-build replacement test preserved durable data, the physical SQLite capacity baseline now covers one attachment at 1 MB, 3 MB and 4.9 MB plus a 4.9 MB record split across three attachments with two queued copies, and the required CI 16 KB emulator runtime suite covers the SQLite/WebView smoke path. The constrained 16 KB emulator intentionally skips the 4.9 MB bridge stress matrix and backend-dependent Truck assertions; those remain normal-runtime/physical-device gates. The plan remains open: final signed-release/physical 16 KB evidence, the full browser race matrix, signed-release APK replacement, broader large-queue/capacity measurements, native reproduction of the original phone error, and release/pilot gates are not yet proven. Editing this document does not authorize production database deployment or data cleanup.

**Current branch-tip evidence:** `a708a9f7` is clean and synchronized with `origin/fix-andriod`. CI run [35512082781](https://github.com/rtemesgen/mathan-apps/actions/runs/35512082781) reports success for frontend, database, browser E2E, normal Android, Android 16 KB runtime, and the required-gates aggregate. The normal Android job includes the host-driven process-death Truck mutation and post-restart server acknowledgement path. The 16 KB job reports a 16,384-byte page size and completes the SQLite instrumentation suite after CI-only background-service/memory stabilization, including the 4 GB emulator memory allocation and disabling the unused Chrome package to prevent WebView memory pressure. This closes the required emulator-CI gate, not final signed-release or physical-device evidence.
...
- Build synthetic Cash Book snapshots with small, 1 MB, 3 MB and near-5 MB attachments, multiple attachments, and a large pending queue. Measure serialized row size, commit/read latency, peak memory, native errors and cold-start success. The physical API 30 baseline covers 1 MB, 3 MB and 4.9 MB single attachments plus a 4.9 MB record split across three attachments with two queued copies; the broader large-queue and peak-memory matrix remains required. Base64 and whole-snapshot duplication mean the file-size limit is not a database-row-size limit.
