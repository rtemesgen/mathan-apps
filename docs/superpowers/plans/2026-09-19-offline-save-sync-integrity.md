...
**Status: implementation in progress; audited checkpoint on `fix-andriod` (2026-09-20).** This revision incorporates the user's detailed requirements supplied after the review. The branch contains implemented and tested portions of phases 0A–8, including native coordination, queue v2 metadata, atomic snapshot acknowledgement, transactional Truck batches, conflict-path wiring, CI-backed browser/database checks, and a backend-connected browser test for the production snapshot conflict “Use server version” UI path. Normal-page physical Android runtime evidence now passes on `SM-N971N` (API 30), the debug previous-build→current-build replacement test preserved durable data, and the physical SQLite capacity baseline now covers one attachment at 1 MB, 3 MB and 4.9 MB plus a 4.9 MB record split across three attachments with two queued copies. The plan remains open: 16 KB runtime evidence, the full browser race/UI matrix, signed-release APK replacement, broader large-queue/capacity measurements, and release/pilot gates are not yet proven. Editing this document does not authorize production database deployment or data cleanup.
